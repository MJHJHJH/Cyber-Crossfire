...
职责：
1. 获取 HotUpdate 程序集：
   - `#if !UNITY_EDITOR`：从 `DefaultRawPackage`（`RawFileBuildPipeline`）用 `LoadAssetAsync<RawFileObject>("HotUpdate.dll")` 加载。寻址名是 `HotUpdate.dll`（`AddressByFileName` = `GetFileNameWithoutExtension`，会去掉 `.bytes`）。`RawFileObject.GetBytes()` → `Assembly.Load(byte[])`。
   - `#else`：`AppDomain.CurrentDomain.GetAssemblies().First(a => a.GetName().Name == "HotUpdate")`。
2. 加载 AOT 补充元数据（仅真机）：
   - `package.GetAllAssetInfos()` 遍历，按 `AssetInfo.AssetPath` 前缀 `Assets/Project/Bundles/HybridCLR/AotMetadata` 过滤，逐个 `LoadAssetAsync<RawFileObject>` 读取 bytes，执行 `RuntimeApi.LoadMetadataForAOTAssembly(byte[], HomologousImageMode.SuperSet)`，按 `LoadImageErrorCode` 校验结果。
   - Editor 模式跳过（AOT 程序集已全量在域内）。
3. 通过反射获取热更入口类型 `GamePlay.ProcedureMain`，存入静态字段 `MainProcedureType`；再 `Activator.CreateInstance` + `ProcedureComponent.AddProcedure` 注册进 Fsm。
...
1. **Addressable location 与加载类型**：`DefaultRawPackage` 走 `RawFileBuildPipeline`，运行时必须 `LoadAssetAsync<RawFileObject>`（不能当 `TextAsset`）。寻址规则 `AddressByFileName` 使 `HotUpdate.dll.bytes` 的 location 为 `HotUpdate.dll`；AotMetadata 同理（如 `mscorlib.dll`）。若收集配置调整需同步常量。
